...
def generate_results_file():
    ...
    if not candidates:
        logger.error("No candidates found in knowledge graph. Please generate datasets first.")
        return
    
    positions = kg.get_all_positions()
    if not positions:
        logger.error("No positions found in knowledge graph. Please generate datasets first.")
        return
    ...
    warm_regret = learning_curves['warm_start']['cumulative_regret']
    cold_regret = learning_curves['cold_start']['cumulative_regret']
    ...
    # Write to file
    output_path = Path(__file__).parent.parent / "results_of_phase_11.txt"
